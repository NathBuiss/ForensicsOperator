...
export default function Timeline({ caseId, artifactTypes }) {
  ...
  return (
    <div className="flex h-full">
      {/* Filter sidebar */}
      <div className="w-44 flex-shrink-0 bg-gray-900/50 border-r border-gray-700/60 flex flex-col">
        ...
        <div className="p-3 border-t border-gray-700/60 space-y-0.5">
          <p className="text-xs font-semibold text-gray-300">{total.toLocaleString()}</p>
          <p className="text-[10px] text-gray-600">{query ? 'search results' : 'events total'}</p>
          {events.length < total && <p className="text-[10px] text-gray-600">{events.length} loaded</p>}
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Search bar */}
        <div className="px-4 py-3 border-b border-gray-700/60 bg-gray-900/30">
          <form onSubmit={submitSearch} className="flex gap-2">
            ...
            {histogram.length > 0 && (
              <button type="button" onClick={() => setShowHistogram(v => !v)}
                className={`btn-ghost text-xs ${showHistogram ? 'text-indigo-400' : ''}`} title="Toggle histogram">
                <BarChart2 size={13} />
              </button>
            )}
          </form>
          {query && (
            <div className="flex items-center gap-2 mt-2">
              ...
              <span className="badge bg-indigo-900/40 text-indigo-300 border border-indigo-700/40 text-[10px]">{query}</span>
              <span className="text-[10px] text-gray-600">— {total.toLocaleString()} result{total !== 1 ? 's' : ''}</span>
            </div>
          )}
        </div>
        ...
        {showHistogram && histogram.length > 0 && (
          <div className="px-4 py-2 border-b border-gray-700/60 bg-gray-900/20">
              ...
              {histogram.map((b, i) => {
                ...
                return (
                  <div key={i} className="flex flex-col items-center group cursor-pointer flex-shrink-0"
                    ...
                    <div style={{ height: h }}
                      className={`w-2 rounded-t transition-colors ${active ? 'bg-indigo-400' : 'bg-indigo-700/60 group-hover:bg-indigo-500/80'}`} />
                  </div>
                )
              })}
            </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {events.length === 0 && !loading && (
            <div className="flex flex-col items-center justify-center h-48 text-center">
              ...
              <p className="text-gray-600 text-xs mt-1">{query ? 'Try a different query.' : 'Upload forensics files in the Ingest tab.'}</p>
            </div>
          )}
          <table className="w-full text-xs">
            <thead className="sticky top-0 bg-gray-900 border-b border-gray-700/60 z-10">
              <tr>
          ...
          <div ref={loaderRef} className="py-5 flex items-center justify-center text-gray-600 text-xs gap-2">
            {loading ? <><Loader2 size={13} className="animate-spin" /> Loading…</>
              : events.length < total ? <span className="text-gray-700">↓ Scroll for more</span>
              : events.length > 0 ? <span className="text-gray-700">— End of results —</span> : null}
          </div>
        </div>
      </div>
...
function EventRow({ event, onSelect, selected }) {
  ...
  return (
    <tr onClick={() => onSelect(event)}
      className={`border-b border-gray-800/40 cursor-pointer transition-colors text-xs
        ${selected ? 'bg-indigo-950/30 border-indigo-800/30' : event.is_flagged ? 'bg-red-950/20 hover:bg-red-950/30' : 'hover:bg-gray-800/40'}`}>
      ...
      <td className="px-3 py-2 text-gray-400 truncate max-w-[7rem]">{event.host?.hostname || ''}</td>
      <td className="px-3 py-2 text-gray-400 truncate max-w-[6rem]">{event.user?.name || ''}</td>
      <td className="px-3 py-2 text-gray-300 max-w-sm"><span className="line-clamp-1">{event.message}</span></td>
      <td className="px-3 py-2">
        <div className="flex items-center gap-1 justify-end">
          {event.is_flagged       && <Flag size={10} className="text-red-400" />}
          {event.tags?.length > 0 && <Tag  size={10} className="text-indigo-400" />}
        </div>
      </td>
    </tr>
  )
