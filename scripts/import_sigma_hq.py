...
Usage:
    ...
    python3 scripts/import_sigma_hq.py --levels critical,high
...
def convert_rule(rule, backend: LuceneBackend) -> Optional[Dict]:
    ...
    try:
        # Resolve rule references first (new Sigma API requirement)
        try:
            from sigma.conversion.state import ConversionState
            conversion_state = ConversionState()
            # Try to resolve references if the method exists
            if hasattr(rule, 'resolve_rule_references'):
                rule.resolve_rule_references()
        except Exception as ref_error:
            logger.debug(f"Could not resolve references for '{rule.title}': {ref_error}")
            # Continue anyway - some rules don't need resolution
        ...
        logger.debug(f"Failed to convert rule '{rule.title}': {e}")
        return None
        ...
        with open(filename, 'w') as f:
            ...
            for item in rules:
                ...
                if meta['sigma_author']:
                    authors_str = ', '.join(str(a) for a in meta['sigma_author'][:3])
                    if len(meta['sigma_author']) > 3:
                        authors_str += '...'
